function PrecomputeCellCheckGUI
    ...
    S.ChooseDatafileButton = uibutton(S.GridLayout,'push','Text','Select .mat File',...
    ...
    function onChooseFolder(~,~)   % [new]
        folder = uigetdir(pwd, 'Select a folder');
        if isequal(folder,0)
            return;
        end
        S.dataFilePath = fullfile(folder);
        S.DataFileEditField.Value = S.dataFilePath;
        tryEnableInputs();
    end
    ...
    function updateDataFileButton()    % [new]
        ...
        case 1   % Extract
            S.ChooseDatafileButton.Text = 'Select .mat File';
            S.ChooseDatafileButton.ButtonPushedFcn = @onChooseMat;
        case 2   % Caiman
            S.ChooseDatafileButton.Text = 'Select .hdf5 File';
            S.ChooseDatafileButton.ButtonPushedFcn = @onChooseHdf5;
        case 3   % Suite2p
            S.ChooseDatafileButton.Text = 'Select Suite2p Folder';
            S.ChooseDatafileButton.ButtonPushedFcn = @onChooseFolder;
        end
    end
        ...
        for i = 1:numJobs
            ...
            try
                ...
                switch job.algorithm
                    ...
                    case 2
                        PrecomputeCellCheck_CaimAn(job.mat, job.h5, ...
                            'output_path', job.output, ...
                            'parallel', job.parallel, ...
                            'fast_features', job.fast, ...
                            'dt', job.dt)
                    ...
                    case 3
                        PrecomputeCellCheck_suite2p(job.mat, job.h5, ...
                            'output_path', job.output, ...
                            'parallel', job.parallel, ...
                            'fast_features', job.fast, ...
                            'dt', job.dt)
