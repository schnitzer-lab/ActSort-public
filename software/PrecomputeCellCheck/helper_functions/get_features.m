...
% Feature 12-29: Calculate event features
if ~fastFeatureFlag
...
% Feature 30-34: Calculate fourier feautres
...
disp("[INFO] -- Temporal Features... DONE!")
...
features(:,f_id) = replace_nans_with_mean(circularities);                 f_id = f_id + 1;
...
if ~fastFeatureFlag
    ...
    spat_corr = compute_spat_corruption(S, [h,w],0,1, parforFlag);
    features(:,f_id) = replace_nans_with_mean(spat_corr); 
    f_id = f_id + 1;
...
disp("[INFO] -- Spatial Features... DONE!")
...
% Features 51-66: Find Spurious Cells
if ~fastFeatureFlag
    ...
    features(:,f_id) = replace_nans_with_mean(corr_score);            f_id = f_id + 1;
    features(:,f_id:f_id+4) = replace_nans_with_mean(scores_1)';       f_id = f_id + 5;
    features(:,f_id:f_id+4) = replace_nans_with_mean(scores_2)';       f_id = f_id + 5;
    features(:,f_id:f_id+4) = replace_nans_with_mean(scores_3)';       f_id = f_id + 5;
...
disp("[INFO] -- Spatiotemporal Features... DONE!")
end

%% helper function
function metric = replace_nans_with_mean(metric)
    % INPUT
    %   [metric] : size N x num_cells
    % OUTPUT
    %   [metric] : size N x num_cells
    % get isinf index
    num_cells = size(metric, 2);
    inf_idx = isinf(metric);
    temp_metric = metric;
    temp_metric(inf_idx) = NaN;
    metric_max = max(temp_metric,[], 2, 'omitnan')*10;
    metric_max = repmat(metric_max, 1, num_cells);
    metric(inf_idx) = metric_max(inf_idx);
    metric_mean = nanmean(metric, 2);
    metric_mean = repmat(metric_mean, 1, num_cells);
    nan_idx = isnan(metric);
    metric(nan_idx) = metric_mean(nan_idx);
