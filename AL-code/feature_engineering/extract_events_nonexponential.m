function event_frames = extract_events_nonexponential(trace,scale,do_smoothing, varargin)
...
thresh1 = scale *max(smooth_trace);% mad(diff_trace(diff_trace>0),1);;
thresh2 = scale *max(diff_trace);
warning off;
% [~,event_frames] = findpeaks(smooth_trace,'minpeakheight',thresh1);
[event_frames, ~] = peakseek(smooth_trace, 3, thresh1);
warning on;
% Filter by diff
...
if ~isempty(varargin)
    if varargin{1} == 1
        ...
        hold on;
        stem(event_frames, scale*ones(1, length(event_frames)), 'Linestyle', 'none');
        plot(scale*ones(1,length(diff_trace)),'r')
